...
public class PlotPanel extends JPanel {
    ...
    public void paintComponent(Graphics g) {
        ...
        g.drawString("E(a, b) = - cos(eta_ab) ;   Total counts: " + results.getTotalcount(), BORDER + 20, 20);
    ...
    public void saveImage(String file) {
        BufferedImage img = getImage(800, 400);
    ...
    public void drawData(Graphics g, double dx, int my, double dy) {
        Color[] colors = {
            Color.red.darker(), Color.white, Color.white, Color.black 
        };
    ...
    public void drawCoordinateSystem(Graphics g, double dx, int my, int w, double dy) {
        ...
        // draw x axis
        g.setColor(Color.black);
        for (double x = 0; x <= MAX_PLOT_ANGLE; x += 10.0) {
            int guix = (int) (x * dx) + BORDER;
            int guiy = (int) (my + 15);
            if (x % 5 == 0) {
        ...
        g.drawString("Angle eta_ab", w / 2 - BORDER, my + 40);
        // draw y axis      
        for (double y = 0-1.0; y <= 1.0; y += 0.5) {
            int guix = BORDER - 20;
            int guiy = (int) (my - y * dy);
